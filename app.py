...
#         PROCESSED / "forecast_output.csv", parse_dates=["week_start"]
#     )
#     risk = pd.read_csv(PROCESSED / "risk_scores.csv")
#     weekly = pd.read_csv(
#         PROCESSED / "weekly_features.csv", parse_dates=["week_start"]
#     )
...
# st.caption(
#     "Demand & Inventory Intelligence | SKU-level planning for NorthBay Living"
# )
...
# sku_options = ["(pick a SKU)"] + sorted(filtered["sku_id"].astype(str).unique())
# sel_sku = st.sidebar.selectbox("SKU", sku_options)

# st.sidebar.markdown("---")
# st.sidebar.caption("Model")
# st.sidebar.write("LightGBM · 8-week horizon")
# st.sidebar.caption("Evaluation")
# st.sidebar.write("Rolling-origin backtest · WAPE")

# # ---------- KPIs ----------
# c1, c2, c3, c4 = st.columns(4)
# c1.metric("SKUs in view", f"{len(filtered):,}")
# c2.metric(
#     "Sales at risk",
#     f"₹{filtered['stockout_value_at_risk'].sum():,.0f}",
# )
# c3.metric(
#     "Capital locked",
#     f"₹{filtered['overstock_capital_locked'].sum():,.0f}",
# )
# c4.metric(
#     "SKUs needing action",
#     f"{(filtered['risk_quadrant'] != 'Healthy').sum():,}",
# )
...
# with left:
#     st.subheader("Risk decision map")
#     fig = px.scatter(
#         filtered,
#         x="overstock_risk",
#         y="stockout_risk",
...
#         st.success("No immediate actions in this view.")
#     else:
#         display = action_list[
#             [
#                 "sku_id",
...
# # ---------- SKU forecast ----------
# st.subheader("Forecast vs historical demand")
...
#     sku_row = risk[risk["sku_id"] == sel_sku].iloc[0]
...
#             y=list(fut["forecast_high_80"]) + list(fut["forecast_low_80"])[::-1],
#             fill="toself",
#             line=dict(width=0),
...
#     b.metric("On-hand", f"{sku_row['on_hand_units']:,.0f}")
#     c.metric("On-order", f"{sku_row['on_order_units']:,.0f}")
#     d.metric("Decision", sku_row["risk_quadrant"])

#     st.info(
#         f"**{sel_sku}** · {sku_row['category']} · "
#         f"{sku_row['recommended_action']} · "
#         f"₹{sku_row['rupee_value_at_stake']:,.0f} value at stake."
#     )
# else:
#     st.info("Select a SKU in the sidebar to inspect its 8-week forecast.")

# st.markdown("---")

# # ---------- Full risk table ----------
# st.subheader("SKU-level risk and action table")
# full = (
#     filtered[
#         [
#             "sku_id",
...
#     )
# )
# st.dataframe(full, use_container_width=True, height=460)
...
# st.caption(
#     "FORESIGHT is a planning decision-support prototype. Forecast intervals are "
#     "planning bands, not statistical guarantees."
# )
